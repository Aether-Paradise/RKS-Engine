...
WILD_BATTLE_TEST("Exp Share(held) gives Experience to mons which did not participate in battle")
{
    ...
    GIVEN {
        ...
        if (item == ITEM_EXP_SHARE)
            EXPECT_GT(GetMonData(&gParties[B_TRAINER_0][1], MON_DATA_EXP), gExperienceTables[GetSpeciesGrowthRate(SPECIES_WYNAUT)][40]);
        else
            EXPECT_EQ(GetMonData(&gParties[B_TRAINER_0][1], MON_DATA_EXP), gExperienceTables[GetSpeciesGrowthRate(SPECIES_WYNAUT)][40]);
    }
}
