...
static void DrawExperienceProgressBar(struct Pokemon *unused)
{
    ...
    {
        u32 expBetweenLevels = gExperienceTables[GetSpeciesGrowthRate(summary->species)][summary->level + 1] - gExperienceTables[GetSpeciesGrowthRate(summary->species)][summary->level];
        u32 expSinceLastLevel = summary->exp - gExperienceTables[GetSpeciesGrowthRate(summary->species)][summary->level];
...
static void PrintNotEggInfo(void)
{
    ...
    if (BXPY_SummaryScreen_HideSpecies(sMonSummaryScreen->mode))
        ...
        PrintTextOnWindowToFitPx(PSS_LABEL_WINDOW_PORTRAIT_SPECIES, GetSpeciesName(GetBaseSpeciesId(summary->species2)), 6, 1, 0, 1, WindowWidthPx(PSS_LABEL_WINDOW_PORTRAIT_SPECIES) - 9);
    else
