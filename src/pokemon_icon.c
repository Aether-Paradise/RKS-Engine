...
u8 CreateTaggedMonIcon(u32 tileTag, u32 paletteTag, enum Species species)
{
    ...
    palette.data = gMonIconPaletteTable[GetSpeciesIconPalIndex(species)].data;
...
{
    FreeSpritePaletteByTag(gMonIconPaletteTable[GetSpeciesIconPalIndex(species)].tag);
}
...
const u8 *GetMonIconTilesByIconType(enum Species species, enum SpeciesIconType iconType)
{
    if (iconType == EGG_ICON)
        return gEggDatas[GetSpeciesEggId(species)].eggIcon;
    if (iconType == FEMALE_ICON)
        return GetSpeciesIconSpriteFemale(species);
