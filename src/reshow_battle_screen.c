...
static bool8 LoadBattlerSpriteGfx(enum BattlerId battler)
{
    if (battler < gBattlersCount)
    {
        ...
        else if (gBattleTypeFlags & BATTLE_TYPE_SAFARI && position == B_POSITION_PLAYER_LEFT)
        {
            ...
            TimeMixBattleSpritePalette(OBJ_PLTT_ID(LoadSpritePaletteWithTag(GetTrainerBackPicPalette(CATCH_TUTORIAL_TRAINER_PIC), GetTrainerPicTag(CATCH_TUTORIAL_TRAINER_PIC, FALSE))));
        else if (!gBattleSpritesDataPtr->battlerData[battler].behindSubstitute)
            BattleLoadMonSpriteGfx(GetBattlerMon(battler), battler);
        else
            BattleLoadSubstituteOrMonSpriteGfx(battler, FALSE);
