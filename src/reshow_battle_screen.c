...
static bool8 LoadBattlerSpriteGfx(enum BattlerId battler)
{
    if (battler < gBattlersCount)
    {
        ...
        if (!IsOnPlayerSide(battler))
        {
            ...
            TimeMixBattleSpritePalette(OBJ_PLTT_ID(LoadSpritePalette(&gTrainerBacksprites[(gSaveBlock2Ptr->playerGender == FEMALE) ? TRAINER_BACK_PIC_PLAYER_FEMALE : TRAINER_BACK_PIC_PLAYER_MALE].palette)));
        else if (gBattleTypeFlags & BATTLE_TYPE_CATCH_TUTORIAL && position == B_POSITION_PLAYER_LEFT)
            TimeMixBattleSpritePalette(OBJ_PLTT_ID(LoadSpritePalette(&gTrainerBacksprites[CATCH_TUTORIAL_TRAINER_PIC_BACK].palette)));
        else if (!gBattleSpritesDataPtr->battlerData[battler].behindSubstitute)
            BattleLoadMonSpriteGfx(GetBattlerMon(battler), battler);
        else
            BattleLoadSubstituteOrMonSpriteGfx(battler, FALSE);
