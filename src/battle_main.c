...
static void BattleStartClearSetData(void)
{
    ...
    gBattleStruct->safariCatchFactor = GetSpeciesCatchRate(GetMonData(&gParties[B_TRAINER_OPPONENT_A][0], MON_DATA_SPECIES)) * 100 / 1275;
...
void SwitchInClearSetData(enum BattlerId battler, struct Volatiles *volatilesCopy)
{
    ...
    if (effect == EFFECT_BATON_PASS)
    {
        ...
        if (IsSpeciesTelekinesisBanned(gBattleMons[battler].species))
