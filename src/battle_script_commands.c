...
void SetMoveEffect(enum BattlerId battlerAtk, enum BattlerId effectBattler, enum MoveEffect moveEffect, const u8 *battleScript, enum SetMoveEffectFlags effectFlags)
{
    ...
    case MOVE_EFFECT_INCINERATE:
        if ((ItemIsBerry(gBattleMons[effectBattler].item)
    ...
    case MOVE_EFFECT_BUG_BITE:
        ...
        else if (ItemIsBerry(gBattleMons[effectBattler].item)
            && abilities[effectBattler] != ABILITY_STICKY_HOLD)
        {
...
static void FinalizeCapture(void)
{
    ...
    if ((GetConfig(B_CRITICAL_CAPTURE_IF_OWNED) >= GEN_9 && GetSetPokedexFlag(BattleMons[gBattlerTarget].species, FLAG_GET_CAUGHT))
        || IsCriticalCapture())
    {
...
{
    NATIVE_ARGS(u8 battler, const u8 *jumpInstr);
    if (ItemIsBerry(gBattleMons[GetBattlerForBattleScript(cmd->battler)].item))
        gBattlescriptCurrInstr = cmd->nextInstr;
    else
        gBattlescriptCurrInstr = cmd->jumpInstr;
}
