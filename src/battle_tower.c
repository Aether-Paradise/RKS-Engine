    ...
    if (!TESTING)
    {
        if (gBattleTypeFlags & (BATTLE_TYPE_LINK | BATTLE_TYPE_RECORDED_LINK))
        {
            ...
            for (i = 0; i < PARTY_SIZE; i++)
            {
                ...
                if (species)
                {
                    SetMonData(&gParties[B_TRAINER_1][i], MON_DATA_EXP, &gExperienceTables[GetSpeciesGrowthRate(species)][enemyLevel]);
            ...
            for (i = 0; i < PARTY_SIZE; i++)
            {
                ...
                if (species)
                {
                    SetMonData(&gParties[B_TRAINER_3][i], MON_DATA_EXP, &gExperienceTables[GetSpeciesGrowthRate(species)][enemyLevel]);
