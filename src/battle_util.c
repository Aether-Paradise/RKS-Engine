...
void HandleAction_WatchesCarefully(void)
{
    ...
    if (IS_FRLG)
    {
        if (gBattleStruct->safariRockThrowCounter != 0)
        {
            ...
            if (gBattleStruct->safariRockThrowCounter == 0)
            {
                gBattleStruct->safariCatchFactor = GetSpeciesCatchRate(GetMonData(gParties[B_TRAINER_OPPONENT_A], MON_DATA_SPECIES)) * 100 / 1275;
...
u32 TrySetCantSelectMoveBattleScript(enum BattlerId battler)
{
    ...
    if (dynamaxBypassCheck && moveEffect == EFFECT_STUFF_CHEEKS && !ItemIsBerry(gBattleMons[battler].item) && GetConfig(B_STUFF_CHEEKS_SELECTABLE) < GEN_CHAMPIONS)
    {
...
u32 CheckMoveLimitations(enum BattlerId battler, u8 unusableMoves, u32 check)
{
    ...
    for (i = 0; i < MAX_MON_MOVES; i++)
    {
        ...
        // Throat Chop
            ...
            unusableMoves |= 1u << i;
        // Stuff Cheeks
        else if (check & MOVE_LIMITATION_STUFF_CHEEKS && moveEffect == EFFECT_STUFF_CHEEKS && !ItemIsBerry(gBattleMons[battler].item) && GetConfig(B_STUFF_CHEEKS_SELECTABLE) < GEN_CHAMPIONS)
            unusableMoves |= 1u << i;
        // Gorilla Tactics
...
u32 GetBattlerAffectionHearts(enum BattlerId battler)
{
    ...
    if (IsSpeciesMegaEvolution(gBattleMons[battler].species)
    ...
    case ABILITYEFFECT_ENDTURN:
        if (IsBattlerAlive(battler))
        {
            ...
            case ABILITY_CUD_CHEW:
                ...
                else if (!gBattleMons[battler].volatiles.cudChew && ItemIsBerry(GetBattlerPartyState(battler)->usedHeldItem))
                {
...
bool32 HasEnoughHpToEatBerry(enum BattlerId battler, enum Ability ability, u32 hpFraction, enum Item itemId)
{
    ...
    if (hpFraction <= 4 && ItemIsBerry(itemId)
         && gBattleMons[battler].hp <= gBattleMons[battler].maxHP / 2
...
static inline u32 CalcMoveBasePowerAfterModifiers(struct DamageContext *ctx)
{
    ...
    case HOLD_EFFECT_LUSTROUS_ORB:
        if (GetBaseSpeciesId(gBattleMons[battlerAtk].species) == SPECIES_PALKIA && (moveType == TYPE_WATER || moveType == TYPE_DRAGON))
            modifier = uq4_12_multiply(modifier, holdEffectModifier);
        break;
    case HOLD_EFFECT_ADAMANT_ORB:
        if (GetBaseSpeciesId(gBattleMons[battlerAtk].species) == SPECIES_DIALGA && (moveType == TYPE_STEEL || moveType == TYPE_DRAGON))
            modifier = uq4_12_multiply(modifier, holdEffectModifier);
        break;
    case HOLD_EFFECT_GRISEOUS_ORB:
        if (GetBaseSpeciesId(gBattleMons[battlerAtk].species) == SPECIES_GIRATINA && (moveType == TYPE_GHOST || moveType == TYPE_DRAGON))
            modifier = uq4_12_multiply(modifier, holdEffectModifier);
        break;
    ...
    case HOLD_EFFECT_OGERPON_MASK:
        if (GetBaseSpeciesId(gBattleMons[battlerAtk].species) == SPECIES_OGERPON)
           modifier = uq4_12_multiply(modifier, UQ_4_12(1.2));
        break;
    default:
        break;
...
static inline u32 CalcAttackStat(struct DamageContext *ctx)
{
    ...
    atkBaseSpeciesId = GetBaseSpeciesId(gBattleMons[battlerAtk].species);
...
static inline bool32 IsBattlerLeekAffected(enum BattlerId battler, enum HoldEffect holdEffect)
{
    ...
    {
        return GetBaseSpeciesId(gBattleMons[battler].species) == SPECIES_FARFETCHD
...
void TryRestoreHeldItems(void)
{
    ...
    for (u32 i = 0; i < PARTY_SIZE; i++)
    {
        if (gBattleStruct->itemLost[B_TRAINER_PLAYER][i].stolen || returnNPCItems)
        {
            ...
            if (ItemIsBerry(lostItem) && GetMonData(&gParties[B_TRAINER_PLAYER][i], MON_DATA_HELD_ITEM) != lostItem)
                lostItem = ITEM_NONE;

            if ((lostItem != ITEM_NONE || returnNPCItems) && !ItemIsBerry(lostItem))
